...
namespace EvaluationFrameworkROS
{
    public class EvaluationFrameworkMain
    {
        ...
        private static MoveItClient moveItClient;
        ...
        public static void Main(string[] _args)
        {
            ...
            {
                /*
                 * COMMUNICATE WITH MOVEIT
                 * LOOP OVER ALL SOCKEL POSITIONS AND GENERATE TRAJECTORIES
                 * AND TERMINATE CLIENT
                 */
                Console.WriteLine("Moving MoveIt Robot To InitialPosition...");
                moveItClient = new MoveItClient();
                Console.WriteLine("Generating MoveIt Trajectories...");
                for (int iD = 0; iD < EvaluationData.NumberOfParameterCombinations; iD++)
                {
                    List<object> parameterCombination;
                    if (EvaluationData.DemonstrationMode)
                        parameterCombination = EvaluationData.DemonstrationEvaluationParameterSetting;
                    else
                        parameterCombination = EvaluationData.EvaluationParameterGrid[iD];

                    moveItClient.PlanCartesianPath(iD,
                                                   -1 * float.Parse((string)parameterCombination[0],
                                                                    EvaluationData.ParsingCultureInfo),
                                                   scalingFactor * float.Parse((string)parameterCombination[1],
                                                                               EvaluationData.ParsingCultureInfo),
                                                   float.Parse((string)parameterCombination[2],
                                                               EvaluationData.ParsingCultureInfo),
                                                   bool.Parse((string)parameterCombination[3]));
                }
                moveItClient.Terminate();
        ...
        private static void SaveROSParameters()
        {
            for (int instanceID = 0; instanceID < EvaluationData.NumberOfParameterCombinations; instanceID++)
            {
                EvaluationData.ROSParameterValues[instanceID] = new object[EvaluationData.NumberOfROSParameters];
                for (int paramID = 0; paramID < EvaluationData.NumberOfROSParameters; paramID++)
                    EvaluationData.ROSParameterValues[instanceID][paramID] = moveItClient.trajectoryDict[instanceID][paramID];
            }

            if (EvaluationData.DemonstrationMode)
                EvaluationData.DemonstrationROSParameterSetting = EvaluationData.ROSParameterValues[0];
        }
